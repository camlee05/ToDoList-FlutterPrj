...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    // 🔥 SỬA: Load tuần tự đảm bảo thứ tự
    _initData().then((_) {
  ...
  // 🔥 THÊM: Hàm load data tuần tự
  Future<void> _initData() async {
    await _loadUsername();
    await _loadTasksFromPrefs();
    await _loadNotifications(); // ĐẢM BẢO LOAD NOTIFICATIONS XONG TRƯỚC
  }
  ...
  Future<void> _saveTasksToPrefs() async {
    ...
    final encoded = jsonEncode(widget.tasks.map((t) {
      ...
      // 🔥 QUAN TRỌNG: Chuyển đổi Color thành int
      ...
      try {
        ...
        setState(() {
          ...
          widget.tasks.addAll(decoded.map<Map<String, dynamic>>((t) {
            ...
            // 🔥 QUAN TRỌNG: Convert int back to Color
            if (map["color"] is int) {
  ...
  Future<void> _saveNotifications() async {
    ...
    // SỬA: Tạo instance của AuthService
    ...
    debugPrint('💾 Bắt đầu lưu ${_notifications.length} notifications cho user $userId');
  ...
  Future<void> _loadNotifications() async {
    ...
    if (jsonStr != null) {
      try {
        ...
        debugPrint('📥 Đã load ${_notifications.length} notifications cho user $userId');
      } catch (e) {
  ...
  void _checkAllTasksForNotifications() {
    final now = DateTime.now();
    final todoTasks = widget.tasks.where((t) => t["done"] == false && t["dueDate"] != null).toList();
    ...
    for (final task in todoTasks) {
      ...
      // 🔥 SỬA: So sánh đơn giản hơn - chỉ kiểm tra taskTitle
      final alreadyNotified = _notifications.any((n) => n["taskTitle"] == task["title"]);
      ...
      if ((missed || overdue) && !alreadyNotified) {
        ...
        debugPrint('❌ KHÔNG tạo thông báo vì: ${alreadyNotified ? "đã thông báo" : "không missed/overdue"}');
      }
    }
  }
  ...
  void _addNotification(String taskTitle, String type, DateTime dueDate) async {
    ...
    await _saveNotifications(); // ĐỢI LƯU XONG
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: SingleChildScrollView( // 🔥 SỬA: BỌC COLUMN BẰNG SINGLECHILDSCROLLVIEW
            child: Column(
  ...
  void _showNotificationsPanel() {
    debugPrint('👁️ Mở notifications panel - Số lượng: ${_notifications.length}');

    // In ra chi tiết từng notification để debug
    for (var noti in _notifications) {
      debugPrint('   📌 ${noti["taskTitle"]} - ${noti["type"]} - read: ${noti["read"]}');
    }
            ...
            Expanded(
              child: _notifications.isEmpty
                ...
                itemBuilder: (context, index) {
                  debugPrint('📌 Hiển thị notification: ${_notifications[index]["taskTitle"]}');
