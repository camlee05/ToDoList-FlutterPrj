...
class NotificationService {
  // ValueNotifier để tự động cập nhật UI khi có thông báo mới
  ...
  static Future<void> showAppNotification({
    ...
    final notis = await getNotifications();
    notifier.value = notis; // cập nhật notifier
  }

  static Future<void> _saveNotificationToLocal(String title, String body) async {
  ...
  static Future<List<Map<String, dynamic>>> getNotifications() async {
    final prefs = await SharedPreferences.getInstance();
    final rawList = prefs.getStringList('notifications') ?? [];
    return rawList.map((e) => jsonDecode(e)).toList().cast<Map<String, dynamic>>();
  }

  static Future<void> clearNotifications() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.remove('notifications');
    notifier.value = []; // cập nhật khi xóa
  }
}
